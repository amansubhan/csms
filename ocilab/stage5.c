...
void query_salary(OCISvcCtx *svchp, OCIError *errhp, thdata *pthdata)
{
  ...
  /* We need to fetch beyond last row, i.e. end of fetch error so that
   * the result-set cached is available for cache matches later
   */ 
  status = OCIStmtFetch(stmthp, errhp, num_rows, OCI_FETCH_NEXT,
                        OCI_DEFAULT);

  if (verbose_flag)
    printf("single row fetch, next fetch status = %d\n",status);
...
void do_workload(OCISvcCtx *svchp, OCIError *errhp, thdata *pthdata, int iteration)
{
  ...
  /* decrease updates for client result caching effect to show in AWR */
  if (iteration % 100 == 0)
